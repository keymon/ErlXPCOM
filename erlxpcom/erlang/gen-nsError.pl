...
# Generates erlxpcom_error.hrl from xpcom/base/nsError.h
# Original code from java extension.
...
while (<STDIN>) {
  ...
  if (/^\s*#define\s+(NS_[A-Z_]+)\s+(0x)?([0-9a-fA-F]+)\s*$/) {
    ...
    $module = $2;
    $code = $3;
    print "-define($1, ((?NS_ERROR_SEVERITY_ERROR bsl 31) | ((?$module+?NS_ERROR_MODULE_BASE_OFFSET) bsl 16) | $code)).\n";
  }
    ...
    $module = $2;
    $code = $3;
    print "-define($1, ((?NS_ERROR_SEVERITY_SUCCESS bsl 31) | ((?$module+?NS_ERROR_MODULE_BASE_OFFSET) bsl 16) | $code)).\n";
  }
  elsif (/^\s*\/\*(.*)\*\/\s*$/ && !/^\s*\/\*@[\{\}]\*\/\s*$/ &&
         !/^\s*\/\*\ -\*- Mode:/) {
